...
async def ban_by_username(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Доступ запрещен")
        return
    ...
    banned_users.add(found_user_id)
    await message.answer(f"✅ Пользователь @{username} (ID: {found_user_id}) заблокирован!")
    
    try:
        await bot.send_message(found_user_id, "🚫 Вы были заблокированы администратором!")
    except:
        pass


@dp.message(Command("unban"))
async def unban_by_username(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Доступ запрещен")
        return
    ...
    if found_user_id in banned_users:
        banned_users.remove(found_user_id)
        await message.answer(f"✅ Пользователь @{username} разблокирован!")
    else:
        await message.answer(f"ℹ️ Пользователь @{username} не был заблокирован")
...
async def banlist_command(message: Message):
    ...
    ban_list = "🚫 Заблокированные пользователи:\n\n"
    for user_id in list(banned_users)[:20]:
        try:
            chat = await bot.get_chat(user_id)
            username = f"@{chat.username}" if chat.username else "Нет username"
            ban_list += f"• {username} (ID: {user_id})\n"
        except:
            ban_list += f"• ID: {user_id} (недоступен)\n"
    
    if len(banned_users) > 20:
        ban_list += f"\n... и еще {len(banned_users) - 20} пользователей"
    
    await message.answer(ban_list)

# ==========================
# 💎 ПОДПИСКА ПО ЮЗЕРНЕЙМУ
# ==========================

@dp.message(Command("add_sub_user"))
async def add_sub_by_username(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Доступ запрещен")
        return
    
    if not command.args:
        await message.answer("❌ Использование: /add_sub_user @username дни")
        return
    ...
    add_subscription(found_user_id, days)
    user = get_user(found_user_id)
    until = user["subscription_until"].strftime("%d.%m.%Y %H:%M")
    
    await message.answer(f"✅ Пользователю @{username} добавлено {days} дней подписки\nДо: {until}")
    
    try:
        await bot.send_message(found_user_id, f"🎉 Вам добавлена подписка на {days} дней!\nДо: {until}")
    except:
        pass


@dp.message(Command("remove_sub_user"))
async def remove_sub_by_username(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Доступ запрещен")
        return
    
    if not command.args:
        await message.answer("❌ Использование: /remove_sub_user @username")
        return
    ...
    await message.answer(f"✅ Подписка пользователя @{username} отменена")


@dp.message(Command("check_sub_user"))
async def check_sub_by_username(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID:
        await message.answer("⛔ Доступ запрещен")
        return
    
    if not command.args:
        await message.answer("❌ Использование: /check_sub_user @username")
        return
    ...
    user = get_user(found_user_id)
    status = "💎 Активна" if has_active_subscription(user) else "❌ Не активна"
    until = user["subscription_until"].strftime("%d.%m.%Y %H:%M") if user["subscription_until"] else "Нет"
    
    await message.answer(
        f"👤 @{username} (ID: {found_user_id})\n"
        f"📊 Статус подписки: {status}\n"
        f"📅 Действует до: {until}\n"
        f"🎯 Чатов сегодня: {user.get('free_chats_today', 0)}/{FREE_CHAT_LIMIT}"
    )
